...
@testset "lon2meters" begin
    x = substitute(
        EarthSciMLBase.lon2meters(0.0), Dict(EarthSciMLBase.lon2m => 40075.0e3 / 360.0))
...
@testset "δxyδlonlat" begin
    ...
    @test isequal(pd,
        Dict(3 => 1.0 / EarthSciMLBase.lat2meters,
            1 => 1.0 / (EarthSciMLBase.lon2m * cos(lat))))
end
...
@testset "DomainInfo" begin
    ...
    have = [δs[i](u) for i in eachindex(δs)]
